  ...
  for (let i = 0; i < splitStr.length; i++) {
    ...
    if (isDecimalMarker(num)) {
      ...
      for (let j = i; j < splitStr.length; j++) {
        const currNum = parseNumStr(splitStr[j]);

        console.log(currNum);
      }

      // if (isHundredsThousands(nextNextNum)) {
      // } else if (isThousands(nextNextNum)) {
...
const isThousands = (nextNextNum) => {
  return nextNextNum && !isDecimalMarker(nextNextNum);
};
// const isHundredsThousands = (nextNextNum) => {};

const isDecimalMarker = (currNum) => {
